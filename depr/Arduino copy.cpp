//Arduino (Old Bootloader) Module that Gathers data from the Endpoint and Sends the Stored Data to the Webserver When Prompted to Do So.
#include <Arduino.h>
#include <DHT.h>
...
static int writeUltrasonicField(char *dest, size_t len, float distanceCm) {
  // Only formats the ultrasonic portion to keep emitJson tight and reusable
  return snprintf(dest, len, "\"distance\":%.2f}", static_cast<double>(distanceCm));
}
...
static void emitJson(const Sample &s) {
  ...
  const int ultrasonicLen = writeUltrasonicField(
      payload + prefixLen, sizeof(payload) - static_cast<size_t>(prefixLen), s.distanceCm);

  if (ultrasonicLen > 0 &&
      static_cast<size_t>(prefixLen + ultrasonicLen) < sizeof(payload)) {
